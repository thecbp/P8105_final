...
Many of the columns have uninuitive or non-descriptive names, so change these up to reflect the information they convey. There are too many columns to rename inline, so we created a function to in `utils.R` to do it compactly in this report. 

```{r mass-renaming }
source("./utils.R")
tidy_colectomies = prettify_names(tidy_colectomies)
```
