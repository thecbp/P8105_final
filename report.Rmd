...
The Ko paper names multiple diseases that are associated with increased risk of SSI. Our dataset contains information on many of these diseases, so we wanted to investigate if our data supports any of Ko's findings. 
...
# cor(colectomy_model) # need more succinct way to show this
# all correlations are below 0.27

fmla = any_ssi ~ surgical_approach + surgical_priority + hypertension + e_diabetes + e_surgical_wound_closure + smoker + bmi + asa_class_id

fit_ssi = glm(fmla, data = model_data)
...
cv_model = train(fmla, data = colecto,
...
<<<<<<< HEAD
make some comments on the regression measures
=======
According to Johns Hopkins School of Medicine's post on surgical site infections (ssi), we look at several potential predictor variables such as Being an elderly adult, being overweight, being a diabetic. smoking status, and cancer status[3]. We further checked the statement and almost all the predictors are significant under 1% significance level except diabetes status, wound closure, surgical approach, and hypertension. However, we include all the predictors in our predictor model for better clinical implications.
>>>>>>> d741cbd4365a81ebebbaa30830ed5028eb7f3201

This regression results give the clinical professionals a detailed look at what pre-surgical disease/traits contribute to ssi. Also, it gives patients more information to decide between different options they are given by the doctors.
